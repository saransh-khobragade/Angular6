...
exports.invite = async (req, res) => {
  console.log(req.body)
	...
	if(myEmail != friendEmail && myEmail != null && myEmail != "" && friendEmail != null && friendEmail != "") {
		User.findOne({myEmail:friendEmail}, function(err, user){
			if (err) {
					return res.json({success: false, message:'Something went wrong'})
			} if(!user){
				return res.json({success: false, message:'friendEmail id is wrong'})
			}
			User.findOne({myEmail}, function(err, user){
				if (err) {
						return res.json({success: false, message:'Something went wrong'})
				} if(user){
					if(user.friendEmails.indexOf(friendEmail)>=0){
						return res.json({success: false, message:'Already friendEmails'})
					} else {
						Notification.findOne({type: 'invite', creater: myEmail, receiver: friendEmail}, function(err, notf){
							if (err) {
								return res.json({success: false, message:'Something went wrong'})
							} else if (notf){
								return res.json({success: false, message:'Request Already sent'})
							} else {
								Notification.findOne({type: 'invite', creater: friendEmail, receiver: myEmail}, function(err, notf){
									if (err) {
										return res.json({success: false, message:'Something went wrong'})
									} else if (notf){
										...
										entry.save(function (err) {
											...
											return res.json({success: true, message:'friendEmail request sent'})
										})
									}
								})
							}
